...
%% Geometry
x = [0:0.2:4,4*ones([1,19]),4:-0.2:0,zeros([1,19])];
y =[zeros([1,21]),0.2:0.2:4,4*ones([1,19]),4:-0.2:0.2];
xo = x;
yo = y;
x(1,end+1) = x(1);
y(1,end+1) = y(1);
...
% patch('faces',edgef(:,1:2),'vertices',nodem, ...
%     'facecolor','w', ...
%     'edgecolor',[.1,.1,.1], ...
%     'linewidth',1.5) ;
[uix,uiy]=InnerDisplacement( x,y,0.4,2,ux,uy,n,nu,mu,f,u );
